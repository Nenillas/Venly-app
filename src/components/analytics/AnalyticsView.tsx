...
export default function AnalyticsView({ month, onMonthChange, entries, getMeta }: Props) {
  ...
  const lineData = months.map((m) => {
    const t = totalsFor(byMonth.get(m) ?? []);
  ...
  return (
    <div className="space-y-6">
      <header className="card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between animate-fade-in">
        ...
        <p className="text-sm text-slate-500">Hälsobetyget gäller vald månads budget. Graferna visar upp till 12 månader.</p>
      </header>
      ...
      {months.length > 0 && (
        <>
          <ChartCard title="Inkomster vs. Utgifter" subtitle="Senaste månaderna · levnadskostnader" icon={<LineIcon className="h-5 w-5 text-emerald-400" />}>
          ...
          <ChartCard title="Sparkvot per månad" subtitle="Andel av inkomsten som sparas" icon={<BarChart3 className="h-5 w-5 text-sky-400" />}>
  ...
  return (
    <section className="card p-5 animate-slide-up">
      <div className="mb-4 flex items-center gap-3">
        <span className="grid h-9 w-9 place-items-center rounded-xl bg-white/5">{icon}</span>
        <div>
